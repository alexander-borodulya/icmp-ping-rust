...
impl Config {
    ...
    pub fn new(
        ...
        let ping_interval =
            Config::check_range(ping_interval, PING_INTERVAL_MIN_MS, PING_INTERVAL_MAX_MS)
                .map_err(|_| ConfigError::PingIntervalOutOfRange(ping_interval))?;
        Ok(Config { target_addr_ipv4, ping_count, ping_interval })
    }
...
impl std::fmt::Display for Config {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(
            f,
            "{}:{}:{}:{}",
            self.target_addr_ipv4.ip(),
            self.target_addr_ipv4.port(),
            self.ping_count,
            self.ping_interval
        )
    }
...
impl FromStr for Config {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let ipv4_s = iter.next().ok_or(ConfigError::ParseError("Column Not Found".to_string()))?;
...
mod tests {
    ...
    fn test_parse_config() {
        ...
        .expect("Config expected to be valid in the test_parse_config unit test");
    ...
    fn test_check_range() {
        ...
        assert_eq!(
            Config::check_range(1, PING_COUNT_MIN, PING_COUNT_MAX),
            Ok(1)
        );
        assert_eq!(
            Config::check_range(10, PING_COUNT_MIN, PING_COUNT_MAX),
            Ok(10)
        );
        assert_eq!(
            Config::check_range(0, PING_COUNT_MIN, PING_COUNT_MAX),
            Err(ConfigError::OutOfRange)
        );
        assert_eq!(
            Config::check_range(11, PING_COUNT_MIN, PING_COUNT_MAX),
            Err(ConfigError::OutOfRange)
        );

        // List of ping interval assertions
        assert_eq!(
            Config::check_range(500, PING_INTERVAL_MIN_MS, PING_INTERVAL_MAX_MS),
            Ok(500)
        );
        assert_eq!(
            Config::check_range(
                PING_INTERVAL_MIN_MS,
                PING_INTERVAL_MIN_MS,
                PING_INTERVAL_MAX_MS
            ),
            Ok(PING_INTERVAL_MIN_MS)
        );
        assert_eq!(
            Config::check_range(
                PING_INTERVAL_MAX_MS,
            ...
            Ok(PING_INTERVAL_MAX_MS)
        );
        assert_eq!(
            Config::check_range(0, PING_INTERVAL_MIN_MS, PING_INTERVAL_MAX_MS),
            Err(ConfigError::OutOfRange)
        );
        assert_eq!(
            Config::check_range(1001, PING_INTERVAL_MIN_MS, PING_INTERVAL_MAX_MS),
            Err(ConfigError::OutOfRange)
        );
    }
    ...
    fn test_parse_config_error() {
        // Bad IPv4 address
        assert_eq!(
            "0.0.0.A,5,1000".parse::<Config>(),
            Err(ConfigError::InvalidIPv4Syntax)
        );

        // Bad ping count number
        assert_eq!(
            "0.0.0.0,B,1000".parse::<Config>(),
            Err(ConfigError::InvalidPingCountSyntax)
        );

        // Bad ping interval number
        assert_eq!(
            "0.0.0.0,5,C".parse::<Config>(),
            Err(ConfigError::InvalidPingIntervalSyntax)
        );

        // Ping count is out of range
        assert_eq!(
            "0.0.0.0,0,1000".parse::<Config>(),
            Err(ConfigError::PingCountOutOfRange(0))
        );

        // Ping interval is out of range
        assert_eq!(
            "0.0.0.0,1,0".parse::<Config>(),
            Err(ConfigError::PingIntervalOutOfRange(0))
        );
    }
}
