...
use socket2::Socket;
use tokio::{time::{self, Instant, Timeout}, task::{self, JoinHandle}, sync::{Mutex, mpsc::{self, error::SendError}}};

use crate::{cli_args::CliArgs, ping_net::PingNet, utils::{self}, connection_handle::{ConnectionHandle, EchoStatus}, config::Config};
...
impl PingApp {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        log::info!("run: identifier: {}, destination: {}, ping_count: {} ping_interval: {}", self.identifier, self.config.target_addr_ipv4.ip(), ping_count, ping_interval);
                ...
                if let Some(seq_num_send) = rx_send.recv().await {
                    ...
                    let send_future_timed_task = task::spawn(async move {
                        ...
                        let send_future = async move {
                            ...
                            let conn_handle = ConnectionHandle::new(destination_ipv4, seq_num_send, identifier, earlier, earlier, EchoStatus::Pending);
                            {
                                let mut conn_map_guard = conn_map.lock().await;
                                let new_entry = conn_map_guard.entry(seq_num_send).or_insert(conn_handle.clone());
                                log::debug!("send_task: new entry: seq_num: {}, status: {}", seq_num_send, new_entry.status);
                            }
                
                            #[cfg(feature = "ENABLE_EMULATE_PAYLOAD")]
                            utils::emulate_payload_delay(seq_num_send).await;
                
                            PingNet::send_ping_request(socket, conn_handle).map_err(RunError::PingNetError).expect("Send Failed");
                
                            let now = Instant::now();
                            ConnectionHandle::new(destination_ipv4, seq_num_send, identifier, earlier, now, EchoStatus::Sent)
                        };
